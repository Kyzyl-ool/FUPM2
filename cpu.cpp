...
Flags(0)
{
	...
	Stack = (int* )std::malloc(STACK_SIZE*sizeof(int));
...
{
	long long int result = (long long)Registers[ri]*Registers[ro];
	Registers[ri+1] = result >> 32;
	Registers[ri] = (result << 32) >> 32;
}

void FUPM_CPU::MULI		(registers r, int number)
{
	long long result = (long long)Registers[r]*number;
	// cout << result << " = " << Registers[r] << " * " << number << endl;
	Registers[r+1] = result >> 32;
	Registers[r] = (result << 32) >> 32;
}
void FUPM_CPU::DIV		(registers ri, registers ro)
{
	long long dividend = Registers[ri+1];
...
{
	long long dividend = Registers[r+1];
	dividend = dividend >> 32;
...
//Реализация не соответствует описанию =)
void FUPM_CPU::CALL		(registers ri, registers ro, int number)
{
	Stack[Registers[r14]--] = Registers[r15];
	Registers[ro] = Registers[ri] + number;
	Registers[r15] = Registers[ri] + number;

	// cout << "I want to jump to " << Registers[r15] << endl;
}
